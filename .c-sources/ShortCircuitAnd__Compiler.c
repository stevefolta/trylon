...
struct ClassInfo ShortCircuitAnd__Compiler__classInfo_ = 
	{ StdClassRef_(Class__CImplementation), 157, 2, Proto_(ShortCircuitAnd__Compiler), Proto_(Compiler), Proto_(TrylonExpression__Compiler), nil, Sym_(ShortCircuitAnd), nil, nil };
...
obj_ compile_co___ShortCircuitAnd__Compiler(obj_ this_, obj_ builder)
{
	obj_ t0_;
	UsingMethod_(compile_short_circuit_and_co_)

		{
		t0_ = Call_(compile_short_circuit_and_co_, builder, this_);
		return t0_;
		}
	return nil;
}
...
obj_ resolve__ShortCircuitAnd__Compiler(obj_ this_)
{
	obj_ t0_;
	obj_ t1_;
