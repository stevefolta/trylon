...
#define new_functions__fld_	(2)
#define is_main__fld_	(3)
...
obj_ function_for_co_in_co___ExistingProto__Compiler(obj_ this_, obj_ selector, obj_ proto)
{
	...
	UsingClass_(ExistingProto__Compiler)
		...
		function_name = t0_;
		t0_ = Call_(index_of_co_, function_name, Int_(0));
		t1_ = Call_(_lt_, t0_, Int_(1));
		if (t1_)
			{
			obj_ result;
			/* A little dangerous: we're going to actually make the call to see if we */
			/* get a proto. */
			t0_ = Call_(perform_co_, proto, selector);
			result = t0_;
			t0_ = result;
			if (t0_) {
				t1_ = Call_(_dt_proto, result);
				t2_ = Call_(same_as_co_, t1_, result);
				t0_ = t2_;
				}
			if (t0_)
				{
				obj_ existing_proto;
				t0_ = Call_(string, selector);
				t1_ = new_co_proto_co___ExistingProto__Compiler(Proto_(ExistingProto__Compiler), t0_, result);
				existing_proto = t1_;
		...
		if (t0_)
			{
			t0_ = new_co_proto_co___ExistingProto__Compiler(Proto_(ExistingProto__Compiler), function_name, proto);
			existing_proto = t0_;
			}
...
obj_ is_main__ExistingProto__Compiler(obj_ this_)
{
	return Field_(is_main);
}


obj_ is_main_co___ExistingProto__Compiler(obj_ this_, obj_ value)
{
...
obj_ lookup_instance_function_co___ExistingProto__Compiler(obj_ this_, obj_ name)
{
	...
	UsingClass_(ExistingProto__Compiler)
		...
		if (t2_)
			{
			obj_ first_char;
			t0_ = Call_(at_co_, name, Int_(0));
			first_char = t0_;
			t0_ = Call_(_gt__eq_, first_char, Int_(1));
			if (t0_) {
				t1_ = Call_(_lt__eq_, first_char, Int_(2));
				t0_ = t1_;
				}
			if (t0_)
				{
...
obj_ name__ExistingProto__Compiler(obj_ this_)
{
	return Field_(name);
}


obj_ name_co___ExistingProto__Compiler(obj_ this_, obj_ value)
{
	Field_(name) = value;
	return value;
}
...
obj_ new_functions__ExistingProto__Compiler(obj_ this_)
{
	return Field_(new_functions);
}


obj_ new_functions_co___ExistingProto__Compiler(obj_ this_, obj_ value)
{
	Field_(new_functions) = value;
	return value;
}
...
obj_ proto__ExistingProto__Compiler(obj_ this_)
{
	return Field_(proto);
}


obj_ proto_co___ExistingProto__Compiler(obj_ this_, obj_ value)
{
	Field_(proto) = value;
	return value;
}
