	...
	UsingClass_(ClassNumberer__CCompiler)
		...
		ForStart_(3, t1_, symbol_info)
			...
			if (t0_)
				...
				t2_ = Call_(_pl_, Str_(13), t1_);
				t3_ = Call_(_pl_, t2_, Str_(14));
...
obj_ emit_dispatch_table_co___CCompiler(obj_ this_, obj_ main_package)
{
	...
	UsingClass_(ExistingFileStream__Standard)
		...
		ForStart_(3, dispatch_table, entry)
			...
			t0_ = Call_(_pl_, num_filled, Int_(2));
			num_filled = t0_;
			}
		ForEnd_(3)
...
obj_ mangle_name_co___CCompiler(obj_ this_, obj_ name)
{
	...
	UsingClass_(String__Standard)
		...
		while (1) {
			...
			if ((t0_) == nil) {
				t2_ = Call_(_gt__eq_, c, Int_(2));
				if (t2_) {
					t3_ = Call_(_lt__eq_, c, Int_(3));
					t2_ = t3_;
					}
				t0_ = t2_;
				}
			if ((t0_) == nil) {
				t4_ = Call_(_gt__eq_, c, Int_(4));
				if (t4_) {
					t5_ = Call_(_lt__eq_, c, Int_(5));
					t4_ = t5_;
					}
				t0_ = t4_;
				}
			...
			if (t6_)
				...
				result = t1_;
				t0_ = Call_(_pl_, p, Int_(6));
				p = t0_;
				run_start = p;
				
				/* Mangle the character. */
				mangled_char = nil;
				t0_ = Call_(_eq__eq_, c, Int_(7));
				if (t0_)
					{
	...
	UsingMethod_(_pl_) UsingMethod_(_lt_) UsingMethod_(_lt__eq_) UsingMethod_(_eq__eq_) UsingMethod_(_gt__eq_) UsingMethod_(deref_char) UsingMethod_(start) UsingMethod_(stopper)
		...
		while (1) {
			...
			if (t0_)
				...
				t0_ = Call_(deref_char, p);
				c = t0_;
				t0_ = Call_(_gt__eq_, c, Int_(3));
				if (t0_) {
					t1_ = Call_(_lt__eq_, c, Int_(4));
					t0_ = t1_;
					}
				if (t0_)
					{
					...
					if (t0_)
						...
						t0_ = Call_(_pl_, p, Int_(7));
						p = t0_;
						}
					else
						{
						/* Only the single character after the `\` is quoted. */
						t0_ = Call_(_pl_, p, Int_(8));
						p = t0_;
						
						}
					}
				}
			t0_ = Call_(_pl_, length, Int_(9));
			length = t0_;
