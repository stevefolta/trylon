...
UsingSym_(ClassParser)UsingClass_(Compiler)
UsingClass_(Object__Standard)
UsingClass_(ClassParser__Compiler)
struct ClassInfo ClassParser__Compiler__classInfo_ = 
	{ StdClassRef_(Class__CImplementation), 106, 5, Proto_(ClassParser__Compiler), Proto_(Compiler), Proto_(Object__Standard), nil, Sym_(ClassParser), nil, nil };
...
obj_ create_co_into_co_context_co___ClassParser__Compiler(obj_ this_, obj_ lines, obj_ proto, obj_ context)
{
	obj_ t0_;
...
obj_ indent__ClassParser__Compiler(obj_ this_)
{
...
obj_ is_name_co___ClassParser__Compiler(obj_ this_, obj_ name)
{
	...
	UsingClass_(ExpressionParser__Compiler)
		...
		{
		t0_ = is_name_co___ExpressionParser__Compiler(Proto_(ExpressionParser__Compiler), name);
		return t0_;
		}
	return nil;
}
...
obj_ new_co_into_co_context_co___ClassParser__Compiler(obj_ this_, obj_ lines, obj_ proto, obj_ context)
{
	UsingClass_(ClassParser__Compiler)
	obj_ obj = AllocObj_(ClassParser__Compiler);
	create_co_into_co_context_co___ClassParser__Compiler(obj, lines, proto, context);
	return obj;
}


obj_ parse__ClassParser__Compiler(obj_ this_)
{
	obj_ t0_;
	obj_ t1_;
...
obj_ parse_arg_type__ClassParser__Compiler(obj_ this_)
{
	obj_ t0_;
	obj_ t1_;
	obj_ t2_;
...
obj_ parse_block_co___ClassParser__Compiler(obj_ this_, obj_ lines)
{
	obj_ t0_;
...
obj_ parse_class_contents_co_into_co___ClassParser__Compiler(obj_ this_, obj_ block, obj_ class_)
{
	...
	UsingClass_(ClassParser__Compiler)
		...
		{
		obj_ parser;
		t0_ = new_co_into_co_context_co___ClassParser__Compiler(Proto_(ClassParser__Compiler), block, class_, class_);
		parser = t0_;
...
obj_ parse_class_spec__ClassParser__Compiler(obj_ this_)
{
	obj_ t0_;
	obj_ t1_;
	obj_ t2_;
	...
	obj_ t4_;
	obj_ t5_;
	DefineString_(0, "Unknown class.")
	DefineString_(1, "Unknown class.")
...
obj_ parse_error_co___ClassParser__Compiler(obj_ this_, obj_ message)
{
	obj_ t0_;
	obj_ t1_;
	obj_ t2_;
...
obj_ parse_error_co_token_co___ClassParser__Compiler(obj_ this_, obj_ message, obj_ token)
{
...
obj_ parse_extension__ClassParser__Compiler(obj_ this_)
{
	obj_ t0_;
	obj_ t1_;
	obj_ t2_;
	...
	DefineString_(1, "...")
	UsingMethod_(_pl_) UsingMethod_(block) UsingMethod_(indent) UsingMethod_(line) UsingMethod_(name) UsingMethod_(parse_class_contents_co_into_co_) UsingMethod_(parse_class_spec) UsingMethod_(report_co_) UsingMethod_(require_eol) UsingMethod_(unindent) UsingMethod_(words)

		{
		obj_ extended_class;
		/*  The class to extend. */
		t0_ = Call_(parse_class_spec, this_);
		extended_class = t0_;
		t0_ = Call_(words, this_);
		t1_ = Call_(require_eol, t0_);
		
		t0_ = Call_(name, extended_class);
		...
		t1_ = Call_(block, t0_);
		t2_ = Call_(parse_class_contents_co_into_co_, this_, t1_, extended_class);
...
obj_ parse_function__ClassParser__Compiler(obj_ this_)
{
	obj_ t0_;
...
obj_ parse_function_co___ClassParser__Compiler(obj_ this_, obj_ initial_name)
{
	obj_ t0_;
	UsingMethod_(parse_function_co_is_primitive_co_)
...
obj_ parse_function_co_is_primitive_co___ClassParser__Compiler(obj_ this_, obj_ initial_name, obj_ is_primitive)
{
	...
	UsingClass_(MethodParser__Compiler)
		...
		if (is_primitive)
			...
			if (t1_)
				...
				t0_ = Call_(line, this_);
				t1_ = Call_(block, t0_);
				t2_ = new_co_in_co___MethodParser__Compiler(Proto_(MethodParser__Compiler), t1_, method_context);
				parser = t2_;
				t0_ = Call_(parse, parser);
				body = t0_;
				}
...
obj_ parse_iff__ClassParser__Compiler(obj_ this_)
{
...
obj_ parse_line_co___ClassParser__Compiler(obj_ this_, obj_ line)
{
	...
	UsingClass_(Main)
		...
		if (SwitchMatches_(Str_(0)) || SwitchMatches_(Str_(1)))
			...
			t0_ = Call_(words, this_);
			t1_ = Call_(next, t0_);
			t0_ = Call_(parse_member_class, this_);
			}
			...
			t0_ = Call_(words, this_);
			t1_ = Call_(next, t0_);
			t0_ = Call_(parse_uses, this_);
			}
		else if (SwitchMatches_(Str_(10)))
			{
			...
			if (t2_)
				...
				t2_ = new_co_line_co_context_co___ExpressionParser__Compiler(Proto_(ExpressionParser__Compiler), t0_, line, t1_);
				parser = t2_;
...
obj_ parse_member_class__ClassParser__Compiler(obj_ this_)
{
	obj_ t0_;
	obj_ t1_;
	obj_ t2_;
	...
	DefineString_(4, "...")
	UsingMethod_(_pl_) UsingMethod_(add_proto_co_) UsingMethod_(block) UsingMethod_(context) UsingMethod_(indent) UsingMethod_(is_name_co_) UsingMethod_(line) UsingMethod_(next) UsingMethod_(parse_class_contents_co_into_co_) UsingMethod_(parse_error_co_) UsingMethod_(report_co_) UsingMethod_(unindent) UsingMethod_(words)

		{
		obj_ member_class, name;
		t0_ = Call_(words, this_);
		t1_ = Call_(next, t0_);
		name = t1_;
		t0_ = Not_(name);
		...
		t0_ = Call_(line, this_);
		t1_ = Call_(block, t0_);
		t2_ = Call_(parse_class_contents_co_into_co_, this_, t1_, member_class);
...
obj_ parse_type_spec__ClassParser__Compiler(obj_ this_)
{
	obj_ t0_;
	obj_ t1_;
...
obj_ parse_uses__ClassParser__Compiler(obj_ this_)
{
	obj_ t0_;
	obj_ t1_;
	UsingMethod_(add_used_namespace_co_) UsingMethod_(parse_class_spec) UsingMethod_(proto) UsingMethod_(require_eol) UsingMethod_(words)
		...
		t1_ = Call_(require_eol, t0_);
		t0_ = Call_(proto, this_);
		t1_ = Call_(add_used_namespace_co_, t0_, used_class);
		}
	return nil;
}


obj_ report_co___ClassParser__Compiler(obj_ this_, obj_ message)
{
...
obj_ unindent__ClassParser__Compiler(obj_ this_)
{
	extern obj_ status_reporter__Main(obj_ this_);
	extern obj_ status_reporter__Main(obj_ this_);
	obj_ t0_;
