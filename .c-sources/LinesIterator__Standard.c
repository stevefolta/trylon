...
struct ClassInfo LinesIterator__Standard__classInfo_ = 
	{ StdClassRef_(Class__CImplementation), 58, 3, Proto_(LinesIterator__Standard), Proto_(Standard), Proto_(Object__Standard), nil, Sym_(LinesIterator), nil, nil };
...
obj_ go_forward__LinesIterator__Standard(obj_ this_)
{
	...
	UsingInt_(1)
	DefineString_(0, "")
	UsingMethod_(_pl_) UsingMethod_(_eq__eq_) UsingMethod_(all_done_co_) UsingMethod_(current_line_co_) UsingMethod_(index_of_co_) UsingMethod_(is_empty) UsingMethod_(remaining_string) UsingMethod_(remaining_string_co_) UsingMethod_(substr_co_) UsingMethod_(substr_co_length_co_) UsingMethod_(unary_minus)
		...
		t1_ = Call_(_eq__eq_, newline_index, t0_);
		if (t1_)
			{
			t0_ = Call_(remaining_string, this_);
