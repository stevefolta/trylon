...
obj_ compile_co___CompiledFunction__Compiler(obj_ this_, obj_ builder)
{
	obj_ t0_;
	obj_ t1_;
	obj_ t2_;
	obj_ t3_;
	UsingMethod_(arguments) UsingMethod_(compile_send_of_co_to_proto_co_with_co_using_co_) UsingMethod_(name) UsingMethod_(on_proto)

		{
		t0_ = Call_(name, this_);
		t1_ = Call_(on_proto, this_);
		t2_ = Call_(arguments, this_);
		t3_ = Call_(compile_send_of_co_to_proto_co_with_co_using_co_, this_, t0_, t1_, t2_, builder);
		return t3_;
		}
	return nil;
}
