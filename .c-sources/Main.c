...
struct ClassInfo Main__classInfo_ = 
	{ StdClassRef_(Class__CImplementation), 3, 0, Proto_(Main), nil, Proto_(Object__Standard), ((obj_) tu0_), Sym_(Main), nil, ((obj_) tu1_) };
struct object Main = 
	{ &Main__classInfo_, {} };
...
obj_ main_co___Main(obj_ this_, obj_ arguments)
{
	...
	UsingClass_(X86Compiler)
		...
		Try_
			{
			obj_ compile_symbols, do_lex_test, do_lines_lexer_test, do_words_lexer_test, file_names, interpret, pass_remaining_args;
			/*  Parse the arguments. */
						...
						if (t0_)
							...
							if (t0_)
								...
								if (t0_)
									...
									{
									t0_ = Call_(_eq__eq_, arg, Str_(6));
									if ((t0_) == nil) {
										t1_ = Call_(_eq__eq_, arg, Str_(7));
										t0_ = t1_;
										}
									if (t0_)
										{
							...
							if (t0_)
								{
									...
									script_path = t0_;
									t0_ = load_co_into_co_as_co___X86Compiler(Proto_(X86Compiler), script_path, Proto_(Main), Str_(10));
								...
								{
								ForStart_(4, file_names, file_name)
									{
