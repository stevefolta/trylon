...
struct ClassInfo Main__classInfo_ = 
	{ 25, 0, Proto_(Main), nil, Proto_(Object__Standard), ((obj_) tu0_), Sym_(Main) ,nil };
struct object Main = 
	{ &Main__classInfo_, {} };

	DefineInt_(0, 1)
static obj_ tu0_[] = { (obj_) StdClassRef_(Tuple), Int_(0), Proto_(Standard) };
	...
	UsingClass_(Standard)
		...
		Try_
			...
			ForStart_(0, t0_, arg)
				...
				if (t0_)
					{
					t0_ = Call_(substr_co_, arg, Int_(1));
	...
	UsingClass_(Standard)
		...
		Try_
			...
			while (1) {
				...
				t0_ = Call_(dump_expr_co_level_co_, this_, expr, Int_(0));
				t0_ = print_line__Standard(Proto_(Standard));
				}
			}
			}
...
obj_ test_lines_lexer_co___Main(obj_ this_, obj_ filename)
{
	...
	UsingClass_(Standard)
		...
		Try_
			...
			t0_ = Call_(lex, lexer);
			lines = t0_;
			t0_ = Call_(dump_lines_co_level_co_, this_, lines, Int_(0));
			}
		TryElse_
			{
			t0_ = Call_(message, exception);
	...
	UsingClass_(WordsLexer__Compiler)
		...
		Try_
			...
			ForStart_(0, t2_, line)
				{
				obj_ lexer;
				t0_ = new_co___WordsLexer__Compiler(Proto_(WordsLexer__Compiler), line);
				lexer = t0_;
				while (1) {
					ContinuePoint_(1)
