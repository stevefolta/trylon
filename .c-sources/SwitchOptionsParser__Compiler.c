	...
	UsingClass_(SwitchClause__Compiler)
		...
		ForStart_(0, t0_, line)
			...
			if (t0_)
				...
				t1_ = new_co_line_co_context_co___ExpressionParser__Compiler(Proto_(ExpressionParser__Compiler), words, line, t0_);
				parser = t1_;
			...
			t3_ = new_co_in_co___MethodParser__Compiler(Proto_(MethodParser__Compiler), t1_, t2_);
			t4_ = Call_(parse, t3_);
			body = t4_;
